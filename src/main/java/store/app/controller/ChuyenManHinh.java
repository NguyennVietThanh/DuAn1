        ...
        private void setChangeBackground(String kind){
            for (DanhMucBeans item : listItem) {
                if (item.getKind().equalsIgnoreCase(kind)) {
                    item.getJlb().setBackground(Color.pink);
                    item.getJpn().setBackground(Color.pink); // khi click vào menu thì sẽ đổi màu panel
                    jlbItem.setForeground(Color.white);
                }else{
                    item.getJlb().setBackground(new Color(0,204,255)); //kh click vào menu thì sẽ giữ nguyên màu lúc ban đầu
                    item.getJpn().setBackground(new Color(0,204,255));
                }
            }
        }
