...
//        model.getMaKH(), 
    ...
    protected List<KhachHang> selectBySqlTenkh(String sql, Object... args) {
        List<KhachHang> list=new ArrayList<>();
        try {
            ResultSet rs = null;
            try {
...
//                    entity.setThanhVien(rs.getBoolean("ThanhVien"));
    ...
    public List<KhachHang> selectByMaKH(String sdt){ //tìm kiếm khách hàng theo từ khóa
        String sql = "SELECT * FROM KhachHang WHERE Sdt LIKE ?";
        return this.selectBySql(sql, "%" + sdt + "%");
    }
    
    public List<KhachHang> selectTenKH() {
        String sql = "SELECT TenKH FROM KhachHang";
        return this.selectBySqlTenkh(sql);
    }
