...
//        model.getMaSP(), 
    ...
    public void update(SanPham model) {
        String sql="UPDATE SanPham SET TenSP =?, maVach=? , SoLuong=? ,DonGiaBan=? ,Anh=? , GhiChu=? ,MaDM = ?, Size = ? WHERE MaSP = ?";
        XJDBC.update(sql, 
        model.getTenSP(), 
        model.getMaVach(),
        model.getSoLuong(),
    ...
    public List<SanPham> selectByMaVach(String keyword){ //tìm kiếm sản phẩm theo từ khóa
        String sql = "SELECT * FROM SanPham WHERE maVach LIKE ?";
        return this.selectBySql(sql, "%" + keyword + "%");
    }
    
//    public List<Object[]> getSanPhamTheomaVach(String maVach){
//        String sql = "{CALL laySanPhamTuMaVach(?)}"; 
//        String[] cols = {"MaSP","TenSP","maVach","SoLuong","DonGiaBan","Anh","GhiChu","MaDM","Size"};// chứa tên cột khi đổ vào table
//        return this.getListOfArray(sql, cols, maVach);//chứa kết quả câu lệnh
//        }
